pipeline {
    ...
    stages {
        ...
        stage ('checkout') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/Amitbaviskar/Java-Project.git']]])
            }
        }
        stage('Build') {
            steps {
        ...
        stage('sonarqube checks') {
            steps {
                script {
                withSonarQubeEnv(installationName: 'sonarqube', credentialsId: 'sonarnewscanner') {
        //          withSonarQubeEnv(credentialsId: 'sonardemo', installationName: 'sonarqube') {
